<x-layouts.dashboard>
    ...
    <div class="flex flex-row items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-heading">Edit Spare Part</h1>
    </div>

    <x-card class="w-full">
        <form action="{{ route('admin.spare-parts.update', $sparePart) }}" method="POST" class="space-y-4" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="flex gap-4">
                ...
                <div class="w-1/2">
                    <x-forms.input label="Unit" name="unit" type="text"
                        placeholder="e.g. pcs"
                        :value="old('unit', $sparePart->unit)" required />
                </div>
            </div>
            ...
            <div class="flex gap-4">
                ...
                <div class="w-1/2">
                    <x-forms.input label="Minimum Stock" name="stock_minimum" type="number"
                        placeholder="e.g. 5"
                        :value="old('stock_minimum', $sparePart->stock_minimum)" required />
                </div>
            </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-default">
                            @foreach(['new', 'used'] as $condition)
                                @php $stock = $stocks[$condition] ?? null @endphp
                                <tr>
                                    <td class="px-4 py-2.5 font-medium text-heading capitalize">{{ $condition }}</td>
                                    <td class="px-4 py-2.5">
                                        <x-forms.input name="stocks[{{ $condition }}][stock]" type="number"
                                            placeholder="0" :value="old('stocks.'.$condition.'.stock', $stock?->stock)" />
                                    </td>
                                    <td class="px-4 py-2.5">
                                        <x-forms.input name="stocks[{{ $condition }}][price]" type="number"
                                            placeholder="0" :value="old('stocks.'.$condition.'.price', $stock?->price)" />
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="flex items-center gap-3">
                <x-button type="submit">Update Spare Part</x-button>
