<x-layouts.dashboard>
    ...
    <div class="flex flex-row items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-heading">Add Category</h1>
    </div>

    <x-card class="w-full">
        <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-4" novalidate>
            @csrf
            <x-forms.input label="Category Name" name="name" type="text"
                placeholder="e.g. Engine" required />
